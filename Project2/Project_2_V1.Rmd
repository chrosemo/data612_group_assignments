...
# Introduction
In this project we will use the built in MovieLense dataset from the recommenderlab .We will implement and compare all the recommendation algorithms given the  project instructions.
...
With reference to code by (Hahsler, 2019), we did some exploration of the dataset so that we get get a sense of the how rating are distributed according to user and item.This will inform how we will subset the data for modeling.
...
Given the skewedness in both these distributions, it is obvious that there is a lot os sparsity in the dataset.
...
It is obvious that there is much sparsity in the dataset.There are many cases where users that might not have rated any movies many movies which may not have been rated In order to have a  healthy baseline on which to our  build recommendation models, we will take into consideration those users who have rated at least 50 movies and those movies that are rated b atleast 100 users. We will then split the data into train and test sets.
