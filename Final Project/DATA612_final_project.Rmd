...
### Train-Test Split
First, we will split the ratings matrix into training and test sets. The training set will be used to build the model and the test set will be used to calculate the accuracy of predictions. 

```{r}
# Splitting into train/test
which_train <- sample(x = c(TRUE, FALSE), 
                      size = nrow(finalRatings), #ratings
                      replace = TRUE, 
                      prob = c(0.8, 0.2))

ratings_train <- finalRatings[which_train,]
ratings_test <- finalRatings[!which_train,]
```

### IBCF

Now we can build our initial item-based recommender. We'll define the following: 
