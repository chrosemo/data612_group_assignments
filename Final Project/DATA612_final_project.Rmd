...
#method = c("cosine", "correlation",  "jaccard", "ejaccard", "dice", "edice", #"hamman","simple matching",  "faith")
...
train_matrix <- as(getData(evalScheme, "train"), "matrix")
test_matrix <- as(getData(evalScheme,"known"), "matrix")

# average for training set
raw_avg <- mean(train_matrix, na.rm =TRUE)
...
# Applying baseline predictor function to each user-item combination of train_matrix
blp <- function(x,y){x + y + raw_avg}

baseline_predictor <- data.frame(
  sapply(1:length(item_bias), function(i) sapply(1:length(user_bias), function(j) blp(user_bias[j], item_bias[i]))), row.names = rownames(train_matrix))

names(baseline_predictor) <- rownames(train_matrix)
colnames(baseline_predictor) <- colnames(train_matrix)

# Clipping max and min values
baseline_predictor[baseline_predictor > 10] <- 10
baseline_predictor[baseline_predictor < -10] <- -10

#################### testing set
baseline_predictor_test <- data.frame(
  t(sapply(rownames(test_matrix), 
           function(i) sapply(rownames(test_matrix), 
                              function(j) blp(user_bias[i], item_bias[j])
                              ))))

names(baseline_predictor_test) <- rownames(test_matrix)
colnames(baseline_predictor_test) <- colnames(test_matrix)

# Clipping max and min values
baseline_predictor_test[baseline_predictor_test > 10] <- 10
baseline_predictor_test[baseline_predictor_test < -10] <- -10
