...
```{r}
# Applying baseline predictor function to each user-item combination
blp <- function(x,y) x + y + raw_avg
baseline_predictor <- data.frame(sapply(1:10, function(i) sapply(1:10, function(j) blp(user_bias[i], item_bias[j]))), row.names = rownames(train_matrix))
