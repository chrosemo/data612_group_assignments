...
recclab_ratings <- as(rating_matrix, 'realRatingMatrix')
```
...
```{r}
jokeCount <- rowCounts(recclab_ratings)
hist(jokeCount,
...
```{r}
ratingCount <- colCounts(recclab_ratings)
hist(ratingCount,
...
```{r}
mean(recclab_ratings@data@x, na.rm = T)
summary(recclab_ratings@data@x, na.rm = T)
hist(recclab_ratings@data@x, main = "Histogram of ratings")
```
...
Next, we train our ALS recommender, setting 10 latent factors and 10 iterations, and use it to predict ratings. Prediction takes a relatively substantial length of time (approximately 4 minutes).
...
```{r}
ratings_df$userID <- 1:nrow(ratings_df)
ratings_df <- ratings_df %>% gather("movieID", "rating", -userID)
ratings_df$movieID <- as.numeric(ratings_df$movieID)
ratings_df$rating <- as.numeric(ratings_df$rating)
ratings_df <- arrange(ratings_df, userID, movieID)
```
...
Next, we build then make predictions using an ALS recommender with parameters that mirror those of the model built with *recommenderlab*.

```{r}
sparklyr_recc <- ml_als(sparklyr_ratings, rating_col = "rating", user_col = "userID", item_col = "movieID", rank = 10, reg_param = 0.1, max_iter = 10)
```
