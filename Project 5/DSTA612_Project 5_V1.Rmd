...
### sparklyr
Next, we move to Spark and *sparklyr*. We start by restructuring the original ratings data and creating a local Spark connection.

```{r}
ratings$userID <- 1:nrow(ratings)
ratings <- ratings %>% gather("movieID", "rating", -userID)
...
We then copy the restructured ratings data into a Spark dataframe.

```{r}
spark_ratings <- sdf_copy_to(sc, ratings, "spark_ratings", overwrite = TRUE)
```
